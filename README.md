...
Resuelve el dilema arquitectónico entre mantener la documentación en el repositorio (para dar contexto como "Docs-as-Code") y tener una base de conocimiento centralizada en Obsidian o cualquier otro directorio. 

Escucha en tiempo real cualquier cambio en los archivos `.md` del código fuente y los copia automáticamente al directorio de destino.
* **Origen (El Código manda):** `<TU_RUTA_DE_ORIGEN>` (ej. `~/Projects/MiRepo`)
* **Destino (Solo lectura):** `<TU_RUTA_DE_DESTINO>` (ej. `~/Documents/Obsidian/Docs`)
...
2. **Reactividad:** Usa `inotifywait` para reaccionar a eventos en tiempo real sin consumir CPU innecesariamente.
3. **Manejo CRUD Completo:** Escucha y replica eventos de creación, modificación, eliminación y renombrado de archivos.
...
**Nota Importante:** Antes de ejecutar el servicio, debes configurar tus rutas reales. Puedes editar las variables `ORIGEN` y `DESTINO` directamente en el script `sync_docs.sh`, o crear un archivo `.env` en el mismo directorio del script con el siguiente contenido:

```env
SYNC_ORIGEN=/ruta/absoluta/a/tu/repo
SYNC_DESTINO=/ruta/absoluta/a/tu/obsidian
```
...
Por defecto, el servicio espera que el script esté en el directorio `~/scripts`. 
```bash
mkdir -p ~/scripts
cp sync_docs.sh ~/scripts/
chmod +x ~/scripts/sync_docs.sh

# Opcional: Copiar archivo .env si decidiste usarlo
# cp .env ~/scripts/
```
