#!/usr/bin/env bash
# --- Sincronización unidireccional Docs-as-Code → Obsidian ---
# Vigila cambios en archivos .md dentro de ORIGEN y los replica en DESTINO.
# Diseñado para correr como systemd user service.
...
# Rutas por defecto (pueden sobreescribirse mediante variables de entorno o .env)
ORIGEN="${SYNC_ORIGEN:-${HOME}/Projects/MyRepo}"
DESTINO="${SYNC_DESTINO:-${HOME}/Documents/Obsidian/Docs}"

LOG_DIR="${HOME}/.local/share/sync-docs"
LOG_FILE="${LOG_DIR}/sync_docs.log"
MAX_LOG_SIZE=$((5 * 1024 * 1024))  # 5 MB

# Regex de exclusión para inotifywait: .git, node_modules, vendor, carpetas ocultas
...
rotate_log() {
    if [[ -f "${LOG_FILE}" ]]; then
        ...
        if (( size > MAX_LOG_SIZE )); then
            ...
            log "LOG ROTATED — archivo anterior guardado como sync_docs.log.old"
        fi
    fi
}

# Verifica si una ruta debe ser ignorada (doble check por si inotifywait deja pasar algo)
should_ignore() {
    local path="$1"
    # Ignorar cualquier ruta que contenga carpetas ocultas, .git, node_modules, vendor
    if [[ "${path}" =~ /\.[^/]+/ ]] || \
       [[ "${path}" =~ /node_modules/ ]] || \
       [[ "${path}" =~ /vendor/ ]]; then
        return 0  # Sí, ignorar
...
# Calcula la ruta destino a partir de la ruta origen
get_dest_path() {
    local src_path="$1"
    # Quitar el prefijo del origen y añadir el prefijo del destino
    echo "${DESTINO}${src_path#"${ORIGEN}"}"
}

# --- Sync Inicial con Rsync ---

initial_sync() {
    log "--- SYNC INICIAL — Replicando estado actual del origen al destino ---"
    ...
    # Limpiar directorios vacíos que rsync pueda dejar en destino
    find "${DESTINO}" -type d -empty -not -path "${DESTINO}/.obsidian*" -delete 2>/dev/null || true

    log "--- SYNC INICIAL COMPLETADO ---"
}
...
handle_event() {
    ...
    # Doble check: ignorar rutas en lista negra
    ...
    if [[ "${events}" == *"ISDIR"* ]]; then
        if [[ "${events}" == *"CREATE"* ]] || [[ "${events}" == *"MOVED_TO"* ]]; then
            log "DIR CREATE/MOVE_TO: ${full_src}"
            mkdir -p "${full_dest}"
        elif [[ "${events}" == *"DELETE"* ]] || [[ "${events}" == *"MOVED_FROM"* ]]; then
            log "DIR DELETE/MOVE_FROM: ${full_src}"
            if [[ -d "${full_dest}" ]]; then
                rm -rf "${full_dest}"
            fi
        fi
    ...
    if [[ "${events}" == *"CREATE"* ]] || \
       ...
       [[ "${events}" == *"MOVED_TO"* ]]; then
        ...
        # Limpiar directorio padre si quedó vacío (no borrar .obsidian)
...
main() {
    ...
    log "--- sync_docs.sh INICIADO ---"
    log "Origen:  ${ORIGEN}"
    log "Destino: ${DESTINO}"
    log "PID:     $$"

    # Sync inicial para cubrir cambios ocurridos offline
    initial_sync

    log "Iniciando monitoreo con inotifywait..."

    # inotifywait en modo monitor (-m), recursivo (-r)
    # Eventos: create, modify, close_write, delete, move
    # Excluir: .git, node_modules, vendor, carpetas ocultas
    inotifywait -m -r \
...
trap 'log "sync_docs.sh DETENIDO (señal recibida)"; exit 0' SIGTERM SIGINT SIGHUP
